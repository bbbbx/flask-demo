...
2. **使用 HTTP 方法描述操作。** 对资源的常见操作：创建、读取、更新、删除（CRUD），HTTP 方法与对应 URL 的关系：
    ...
    |POST    |返回指定数据新地址的表现层，头部的 `Location` 字段为指向资源的 URL，201（Created）响应|
    |PUT     |若包含请求处理状态的表现出，则返回 200 响应；若空数据，则返回 204（No Content）响应|
    |PATCH   |同上  |
    |DELETE  |若请求被接收，但删除操作还未执行，则返回 202（Accepted）响应；若删除操作已经执行，返回 204 响应；若删除操作已经执行，且返回包含状态信息的表现层，则返回 200 响应|
    ...
    **这里的表现层（repersentation）指的资源的表现形式，例如 JSON 格式的数据。**
    ...
    |:------------|:------|:--|
    |200          |OK（成功）|请求成功|
    |201          |Created（已创建）|请求成功，并创建了一个新的资源|
    |202          |Accepted（已接收）|请求已接收，但仍在处理，将异步处理|
